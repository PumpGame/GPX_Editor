...
# -*- coding: utf-8 -*-
"""
GPXEditor â€” single-file GPX track editor with:
...
class GPXEditor:
    ...
    def save_gpx(self):
        ...
        path = os.path.abspath(self.current_path) if self.current_path else ""
        if not path:
    ...
    def save_gpx_as(self):
        if not self.gpx_loaded:
            print("⚠️ Open a GPX file first")
            return
        default_name = os.path.basename(self.current_path) if self.current_path else "edited.gpx"
    ...
    def _on_press(self, event):
        ...
        # LPM â€“ wybÃ³r/przeciÄ…ganie
        if event.button == MouseButton.LEFT:
            idx, dist = self._nearest_index(event.xdata, event.ydata)
            if idx is None:
                return
            if dist is None or dist > self.pick_tolerance_m:
                # klik w puste: wyczyÅ›Ä‡ selekcjÄ™ (chyba Å¼e trzymasz Shift/Ctrl)
            ...
            # Zapisz klikniÄ™cie i wystartuj przeciÄ…ganie dopiero po progu
            self.pending_drag = True
            self.press_idx = idx
    ...
    def _on_motion(self, event):
        ...
        if (
            ...
            if (dx_px * dx_px + dy_px * dy_px) >= 16:  # 4px prÃ³g
        ...
        # przeciÄ…ganie zaznaczonych LPM
    ...
    def _update_plot(self, full=False):
        ...
        # info box â€“ odtwÃ³rz box po clear()
        self._set_title()
        self.fig.canvas.draw_idle()
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _on_tab_bar_changed(self, index):
        if self.tabs is None:
            return
        if 0 <= index < self.tabs.count() and self.tabs.currentIndex() != index:
            self.tabs.setCurrentIndex(index)

    def _on_tab_bar_moved(self, from_index, to_index):
        if self.tabs is None or from_index == to_index:
            return
        widget = self.tabs.widget(from_index)
        text = self.tabs.tabText(from_index)
        tip = self.tabs.tabToolTip(from_index)
        self.tabs.removeTab(from_index)
        new_index = self.tabs.insertTab(to_index, widget, text)
        self.tabs.setTabToolTip(new_index, tip)
    ...
    def _sync_tab_bar_from_tabs(self):
        if not hasattr(self, "tab_bar") or self.tabs is None:
            return
        self.tab_bar.blockSignals(True)
        ...
        for i in range(self.tabs.count()):
            ...
            self.tab_bar.addTab(tab_text)
            self.tab_bar.setTabToolTip(i, self.tabs.tabToolTip(i))
        self.tab_bar.setCurrentIndex(self.tabs.currentIndex())
        self.tab_bar.blockSignals(False)
    ...
    def _build_ui(self):
        ...
        self.tab_bar.tabMoved.connect(self._on_tab_bar_moved)

        tab_bar_row = QtWidgets.QWidget()
        tab_bar_row_layout = QtWidgets.QHBoxLayout(tab_bar_row)
        tab_bar_row_layout.setContentsMargins(0, 0, 0, 0)
        tab_bar_row_layout.setSpacing(6)
        add_tab_btn = QtWidgets.QToolButton()
        add_tab_btn.setText("+")
        add_tab_btn.setToolTip("New tab")
        add_tab_btn.clicked.connect(self._new_empty_tab)
        tab_bar_row_layout.addWidget(self.tab_bar)
        tab_bar_row_layout.addWidget(add_tab_btn)
        tab_bar_row_layout.addStretch(1)
        main_layout.addWidget(tab_bar_row)
        ...
        btn_save_as.clicked.connect(lambda: self.editor.save_gpx_as())
        ...
        file_header = QtWidgets.QLabel("File")
        file_header.setProperty("class", "section-header")
        ...
        left_layout.addWidget(btn_save_simple_oneline)

        sel_header = QtWidgets.QLabel("Selection")
        sel_header.setProperty("class", "section-header")
        self.sel_group = QtWidgets.QButtonGroup(self)
        self.sel_group.setExclusive(True)
        self.btn_sel_point = QtWidgets.QPushButton("")
        self.btn_sel_rect = QtWidgets.QPushButton("")
        self.btn_sel_lasso = QtWidgets.QPushButton("")
        for btn, tip, fallback in (
            (self.btn_sel_point, "Single point", "•"),
            (self.btn_sel_rect, "Rectangle", "▭"),
            (self.btn_sel_lasso, "Lasso", "∿"),
        ):
            btn.setCheckable(True)
            btn.setToolTip(tip)
            btn.setProperty("class", "sel-mode")
            btn.setText(fallback)
        icon_specs = (
            (self.btn_sel_point, "icons/point.svg"),
            (self.btn_sel_rect, "icons/rectangle.svg"),
            (self.btn_sel_lasso, "icons/lasso.svg"),
        )
        for btn, rel_path in icon_specs:
            icon_path = os.path.join(APP_BASE_DIR, rel_path)
            if os.path.isfile(icon_path):
                icon = QtGui.QIcon(icon_path)
                if not icon.isNull():
                    btn.setIcon(icon)
                    btn.setText("")
        self.btn_sel_point.setChecked(True)
        self.sel_group.addButton(self.btn_sel_point, 0)
        self.sel_group.addButton(self.btn_sel_rect, 1)
        self.sel_group.addButton(self.btn_sel_lasso, 2)
        self.sel_group.idToggled.connect(self._on_selection_mode_button_toggled)
        sel_row = QtWidgets.QHBoxLayout()
        sel_row.addWidget(self.btn_sel_point)
        sel_row.addWidget(self.btn_sel_rect)
        sel_row.addWidget(self.btn_sel_lasso)
        self.pick_tolerance_input = QtWidgets.QDoubleSpinBox()
        self.pick_tolerance_input.setRange(0.5, 50.0)
        self.pick_tolerance_input.setDecimals(1)
        self.pick_tolerance_input.setSingleStep(0.5)
        ...
        radius_row.addWidget(self.pick_tolerance_input)
        ...
        cut_input.setRange(1, 9999)
        cut_input.setValue(10)
        cut_input.setAlignment(QtCore.Qt.AlignCenter)
        btn_cut_x_end = QtWidgets.QPushButton("Remove ››")
        btn_cut_x_end.setProperty("class", "danger")
        btn_cut_x_end.clicked.connect(lambda: self.editor._remove_x_from("end"))
        bulk_row.addWidget(btn_cut_x_start)
        bulk_row.addWidget(cut_input)
        bulk_row.addWidget(btn_cut_x_end)
        bulk_label = QtWidgets.QLabel("Remove X points from start / end")
        bulk_label.setAlignment(QtCore.Qt.AlignCenter)
        bulk_label.setStyleSheet("font-size: 10px; color: #868e96;")
        left_layout.addWidget(edit_header)
    ...
    def _build_toolbar(self):
        ...
        self.toolbar.addAction(
            ...
            "",
            self._refresh_current_gpx,
        ).setToolTip("Reload file from disk")
        self.toolbar.addSeparator()
        self.toolbar.addAction(
        ...
        recent_label.setStyleSheet("font-size: 11px; color: #868e96;")
        self.toolbar.addWidget(recent_label)
        self.recent_files_combo = QtWidgets.QComboBox()
        self.recent_files_combo.setMinimumWidth(150)
    ...
    def _set_selection_mode(self, mode):
        ...
        if hasattr(self, "sel_group"):
            mode_to_id = {"Single point": 0, "Rectangle": 1, "Lasso": 2}
            target_id = mode_to_id.get(mode, 0)
            button = self.sel_group.button(target_id)
            if button is not None and not button.isChecked():
                self.sel_group.blockSignals(True)
                button.setChecked(True)
                self.sel_group.blockSignals(False)
        editor = self._active_editor()
        if editor is not None:
            self._apply_selection_mode_to_editor(editor)
        self._update_status_bar()

    def _on_selection_mode_button_toggled(self, mode_id, checked):
        if not checked:
            return
        mode = {0: "Single point", 1: "Rectangle", 2: "Lasso"}.get(mode_id, "Single point")
