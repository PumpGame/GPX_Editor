...
def get_global_output_filename(base_name):
    base = base_name.replace(".txt", "_gps.gpx").replace(".gpx", "_cleaned.gpx")
...
def find_input_files():
    files = [f for f in os.listdir() if f.endswith(".txt") or f.endswith(".gpx")]

    if not files:
        print("[!] Nie znaleziono plików .txt / .gpx")
        exit()
